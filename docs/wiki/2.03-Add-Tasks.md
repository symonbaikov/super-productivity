...
| ------------ | -------- | --------------------------- |
| **Project**  | `Ctrl+4` | Choose the target project   |
| **Schedule** | `Ctrl+5` | Set a due date/time         |
| **Tags**     | `Ctrl+6` | Assign tags                 |
| **Estimate** | `Ctrl+7` | Set a time estimate         |
| **Repeat**   | `Ctrl+8` | Configure a repeat schedule |

Additionally, you can press **Ctrl+9** to open the deadline dialog directly, or click the deadline chip once it has been parsed from the title.
...
- **Schedule** (`Ctrl+5`): _When you plan to work on the task_ — places it in your daily plan.
- **Deadline** (`Ctrl+9`): _When the task must be done by_ — the hard due date. Tasks with a deadline set to today are automatically moved into Today's list.

## Add to top or Bottom

Next to the input there is a button that toggles whether new tasks go to the **top** or **bottom** of the list. The tooltip shows “Add to top (Ctrl+3)” or “Add to bottom (Ctrl+3)”. You can also press **Ctrl+3** to toggle. This applies to the next task you add.
...
Click the **search** icon (or press **Ctrl+1**) in the add task bar to switch to **issue search** mode. The placeholder changes to “Add existing task or issues…”. You can search for:
...
Type to search, then choose an item from the list to add it. Disable search again with the search button or **Ctrl+1** to return to normal “create new task” mode.

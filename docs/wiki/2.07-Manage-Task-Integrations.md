...
**From the Planner or Schedule view:** Right-click (or use the context-menu button on) a calendar event to get two creation options:

- **Create as task** — creates a task linked to the calendar event (it keeps the event's date/time and disappears from the calendar list). Linked tasks cannot be turned into sub-tasks.
- **Create as task (no calendar link)** — creates a plain, unlinked task from the event's title, description (as notes), and duration (as time estimate). It has no date/time, so it can be dragged into another task as a sub-task. The calendar event stays in the list.
